...
namespace COE000.Portal.NomeProjeto.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        private string[] WantedColumns { get; set; }

        private string CsvColumnsFormatRow { get; set; }


        public HomeController(ILogger<HomeController> logger, DataBaseContext context, IConfiguration config)
        {
            _logger = logger;
            _repository = new(context);
        }

        public async Task<IActionResult> Index() => View(new { NotifyModal = new NotifyModel() });
